...
export default function Portfolio(props: ItemProps) {
  ...
  for(let each_market of markets){
    ...
    return (
        <Cover>
          <LeftCover>
            <Icon source={item.asset === 'ETH' ?  etherum : item.asset === 'USD' ? tribl : bitcoin} />
            <Text
              style={{
                color: colors.BLACK,
            ...
            >
              {item.asset === 'ETH' ? t(`community.currency.etherum`) : item.asset === 'USD' ?  t(`community.currency.tribl`) : t(`community.currency.bitcoin`)}
            </Text>
          </LeftCover>
          <RightCover style={{}}>
            <Text
